...
- Today is {today}."""
    ...
    async def search_cmi_restricted(self, query: str):
        """
        Performs a search in the CMi documentation (including private documentation) with the given `query`.
        Returns a list of the document chunks that best match the query.
        """

        print("[CMI RESTRICTED TOOL]", f"Called the `search_cmi_restricted` tool with query=`{query}`")

        gac = GraphAIClient()
        results = await gac.rag_retrieve(index=self.index, texts=[query])

        print("[CMI RESTRICTED TOOL]", f"Retrieved {len(results)} document chunks.")

        return results

    def build_tools(self):
        # Wrap the bound method at runtime
        rag_tool = tool("search_cmi_restricted")
        return [rag_tool(self.search_cmi_restricted)]
